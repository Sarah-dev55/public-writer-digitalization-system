 ...
 */
export default function UpcomingAppointments({ appointments = [], onStatusUpdate = () => {} }) {
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              {(apt.status === 'scheduled' || apt.status === 'confirmed' || !apt.status) && (
                <button
                  onClick={() => onStatusUpdate && onStatusUpdate(apt._id || apt.id, 'completed')}
                  className="p-1.5 text-app-secondary hover:bg-app-secondary/10 rounded-full transition-colors"
